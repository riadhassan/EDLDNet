...
class EDecoder(nn.Module):
    def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, ag_ks=3, activation='relu6'):
        super(EDecoder,self).__init__()
        ucb_ks = 3 # kernel size for ucb
        ...
        self.ucb3 = UCB(in_channels=channels[0], out_channels=channels[1], kernel_size=ucb_ks, stride=ucb_ks//2)
        self.ag3 = AG(F_g=channels[1], F_l=channels[1], F_int=channels[1]//2, kernel_size=ag_ks, groups=channels[1]//2)
        ...
        self.ucb2 = UCB(in_channels=channels[1], out_channels=channels[2], kernel_size=ucb_ks, stride=ucb_ks//2)
        self.ag2 = AG(F_g=channels[2], F_l=channels[2], F_int=channels[2]//2, kernel_size=ag_ks, groups=channels[2]//2)
        ...
        self.ucb1 = UCB(in_channels=channels[2], out_channels=channels[3], kernel_size=ucb_ks, stride=ucb_ks//2)
        self.ag1 = AG(F_g=channels[3], F_l=channels[3], F_int=int(channels[3]/2), kernel_size=ag_ks, groups=int(channels[3]/2))
