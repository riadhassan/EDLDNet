...
def dice_loss(pred, target, smooth=1e-6):
    ...
    Parameters:
    - pred: Tensor of predicted values (logits or probabilities) with shape [batch_size, 1, H, W].
    - target: Tensor of ground truth binary values (0 or 1) with shape [batch_size, 1, H, W].
    - smooth: Smoothing factor to avoid division by zero.

    Returns:
    - The computed Dice loss.
    """
    # Ensure the predictions are in the range [0, 1] (for probabilities)
    ...
    # Compute the Dice score (1 - Dice loss)
...
def print_model_stats(model, input_size=(3, 256, 256)):
    ...
    print(f'Model computation requirement: {macs}')