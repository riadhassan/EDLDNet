...
from network_lib.networks import EDLDNet
from trainer import trainer_synapse
from utils.utils import print_model_stats

parser = argparse.ArgumentParser()

parser.add_argument('--root_path', type=str,
                    default='data\synapse\\train_npz_new', help='root dir for data')
parser.add_argument('--volume_path', type=str,
                    default='data\synapse\\test_vol_h5_new', help='root dir for validation volume data')
...
parser.add_argument('--encoder', type=str,
                    default='pvt_v2_b2', help='Name of encoder: pvt_v2_b2, pvt_v2_b0')
parser.add_argument('--expansion_factor', type=int,
                    default=2, help='expansion factor')
parser.add_argument('--kernel_sizes', type=int, nargs='+',
                    default=[1, 3, 5], help='multi-scale kernel sizes in MSDC block')
parser.add_argument('--ag_ks', type=int,
                    default=3, help='Kernel size in AG')
...
if __name__ == "__main__":
    ...
    run = "EDLDNet"
    args.exp = args.encoder + '_EDLDNet_kernel_sizes_' + str(args.kernel_sizes) + '_dw_' + dw_mode + '_' + aggregation + '_ag_ks_' + str(args.ag_ks) + '_ef' + str(args.expansion_factor) + '_act_mscb_' + args.activation_mscb + '_loss_' + args.supervision + '_output_final_layer_Run'+str(run)+'_' + dataset_name + str(args.img_size)
    snapshot_path = "model_pth/train/{}/{}".format(args.exp, args.encoder + '_EDLDNet_kernel_sizes_' + str(args.kernel_sizes) + '_dw_' + dw_mode + '_' + aggregation + '_ag_ks_' + str(args.ag_ks) + '_ef' + str(args.expansion_factor) + '_act_mscb_' + args.activation_mscb + '_loss_' + args.supervision + '_output_final_layer_Run'+str(run))
    ...
    model = EDLDNet(num_classes=args.num_classes, kernel_sizes=args.kernel_sizes, expansion_factor=args.expansion_factor, dw_parallel=not args.no_dw_parallel, add=not args.concatenation, ag_ks=args.ag_ks, activation=args.activation_mscb, encoder=args.encoder, pretrain= not args.no_pretrain)
    model.cuda()
